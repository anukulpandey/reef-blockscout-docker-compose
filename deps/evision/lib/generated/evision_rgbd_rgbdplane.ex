defmodule Evision.RGBD.RgbdPlane do
  ...
  def apply([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:points3d,:plane_coefficients,:normals,:mask])
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:sensor_error_a,:method,:threshold,:min_size,:block_size,:sensor_error_b,:sensor_error_c])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
