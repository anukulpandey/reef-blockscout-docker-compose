defmodule Evision.Segmentation.IntelligentScissorsMB do
  ...
  def applyImageFeatures([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:image,:gradient_magnitude,:non_edge,:gradient_direction])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  **Note**: buildMap() must be called before this call
  ...
  def getContour([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:contour,:backward,:targetPt])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  **Note**: "Laplacian Zero-Crossing" feature extractor is used by default (following to original article)
  ...
  def setEdgeFeatureCannyParameters([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:apertureSize,:threshold1,:L2gradient,:threshold2])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  **Note**: Thresholding should be used for images with irregular regions (to avoid stuck on parameters from high-contract areas, like embedded logos).
  ...
  def setWeights([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:weight_gradient_direction,:weight_non_edge,:weight_gradient_magnitude])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
