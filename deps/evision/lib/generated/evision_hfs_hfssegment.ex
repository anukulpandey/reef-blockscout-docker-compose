defmodule Evision.HFS.HfsSegment do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:width,:segEgbThresholdI,:spatialWeight,:segEgbThresholdII,:minRegionSizeII,:minRegionSizeI,:height,:numSlicIter,:slicSpixelSize])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def performSegmentCpu([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:ifDraw,:src])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
