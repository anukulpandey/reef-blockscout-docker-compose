defmodule Evision.XFeatures2D do
  ...
  **Note**: 
  ...
  def matchGMS([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:keypoints1,:matches1to2,:size2,:matchesGMS,:withScale,:withRotation,:size1,:keypoints2,:thresholdFactor])
  ...
  **Note**: 
  ...
  def matchLOGOS([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:nn1,:keypoints1,:matches1to2,:nn2,:keypoints2])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
