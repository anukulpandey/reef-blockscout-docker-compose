defmodule Evision.ML.SVM do
  ...
  def getDecisionFunction([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:svidx,:alpha,:i])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def trainAuto([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:samples,:Cgrid,:layout,:balanced,:responses,:gammaGrid,:kFold,:pGrid,:coeffGrid,:degreeGrid,:nuGrid])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
