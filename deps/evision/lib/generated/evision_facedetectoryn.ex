defmodule Evision.FaceDetectorYN do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:bufferModel,:input_size,:backend_id,:target_id,:score_threshold,:nms_threshold,:framework,:config,:model,:bufferConfig,:top_k])
  ...
  def detect([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:image,:faces])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
