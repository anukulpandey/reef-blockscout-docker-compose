defmodule Evision.PPFMatch3D do
  ...
  def computeNormalsPC3d([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:FlipViewpoint,:PCNormals,:NumNeighbors,:PC,:viewpoint])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def samplePCByQuantization([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:yrange,:zrange,:pc,:weightByCenter,:sample_step_relative,:xrange])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
