defmodule Evision.XFeatures2D.MSDDetector do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:m_patch_radius,:m_nms_scale_radius,:m_kNN,:m_th_saliency,:m_n_scales,:m_scale_factor,:m_compute_orientation,:m_search_area_radius,:m_nms_radius])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
