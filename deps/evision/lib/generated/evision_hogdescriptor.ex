defmodule Evision.HOGDescriptor do
  ...
  def hogDescriptor([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:_nbins,:_winSigma,:_L2HysThreshold,:_blockSize,:_histogramNormType,:_winSize,:_blockStride,:_signedGradient,:filename,:_derivAperture,:_nlevels,:_gammaCorrection,:_cellSize])
  ...
  def compute([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:padding,:locations,:img,:descriptors,:winStride])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def computeGradient([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:grad,:paddingBR,:angleOfs,:paddingTL,:img])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def detect([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:weights,:hitThreshold,:padding,:searchLocations,:foundLocations,:img,:winStride])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def detectMultiScale([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:groupThreshold,:hitThreshold,:padding,:foundWeights,:useMeanshiftGrouping,:foundLocations,:img,:scale,:winStride])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def load([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:objname,:filename])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
