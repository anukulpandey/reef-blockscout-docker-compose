defmodule Evision.PPFMatch3D.ICP do
  ...
  def iCP([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:numLevels,:numMaxCorr,:iterations,:tolerence,:sampleType,:rejectionScale])
  ...
  def registerModelToScene([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:srcPC,:pose,:poses,:residual,:dstPC])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
