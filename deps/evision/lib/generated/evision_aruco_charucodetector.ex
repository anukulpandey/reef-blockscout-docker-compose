defmodule Evision.ArUco.CharucoDetector do
  ...
  def charucoDetector([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:charucoParams,:refineParams,:board,:detectorParams])
  ...
  **Note**: After OpenCV 4.6.0, there was an incompatible change in the ChArUco pattern generation algorithm for even row counts.
  ...
  def detectBoard([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:image,:markerIds,:charucoIds,:charucoCorners,:markerCorners])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  **Note**: After OpenCV 4.6.0, there was an incompatible change in the ChArUco pattern generation algorithm for even row counts.
  ...
  def detectDiamonds([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:image,:markerIds,:diamondIds,:diamondCorners,:markerCorners])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
