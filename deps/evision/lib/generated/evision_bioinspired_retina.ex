defmodule Evision.Bioinspired.Retina do
  ...
  def applyFastToneMapping([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:inputImage,:outputToneMappedImage])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:inputSize,:samplingStrength,:reductionFactor,:useRetinaLogSampling,:colorSamplingMethod,:colorMode])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def setup([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:applyDefaultSetupOnFailure,:retinaParameterFile])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def setupIPLMagnoChannel([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:parasolCells_k,:localAdaptintegration_tau,:parasolCells_tau,:parasolCells_beta,:localAdaptintegration_k,:amacrinCellsTemporalCutFrequency,:V0CompressionParameter,:normaliseOutput])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def setupOPLandIPLParvoChannel([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:HcellsSpatialConstant,:HcellsTemporalConstant,:horizontalCellsGain,:photoreceptorsSpatialConstant,:ganglionCellsSensitivity,:photoreceptorsTemporalConstant,:colorMode,:photoreceptorsLocalAdaptationSensitivity,:normaliseOutput])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
