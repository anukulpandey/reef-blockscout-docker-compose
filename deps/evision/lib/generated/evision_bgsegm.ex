defmodule Evision.BgSegm do
  ...
  def createBackgroundSubtractorCNT([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:isParallel,:minPixelStability,:maxPixelStability,:useHistory])
  ...
  def createBackgroundSubtractorGSOC([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:noiseRemovalThresholdFacFG,:noiseRemovalThresholdFacBG,:replaceRate,:alpha,:hitsThreshold,:blinkingSupressionDecay,:mc,:nSamples,:propagationRate,:blinkingSupressionMultiplier,:beta])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def createBackgroundSubtractorLSBP([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:noiseRemovalThresholdFacFG,:noiseRemovalThresholdFacBG,:Tupper,:LSBPthreshold,:Tdec,:mc,:Rscale,:Tlower,:Tinc,:minCount,:nSamples,:LSBPRadius,:Rincdec])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def createBackgroundSubtractorMOG([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:backgroundRatio,:history,:nmixtures,:noiseSigma])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def createSyntheticSequenceGenerator([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:object,:wavespeed,:wavelength,:background,:objspeed,:amplitude])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
