defmodule Evision.DNN.Image2BlobParams do
  ...
  def image2BlobParams([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:ddepth,:borderValue,:swapRB,:datalayout,:mode,:scalefactor,:mean,:size])
  ...
  def blobRectsToImageRects([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:size,:rImg,:rBlob])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
