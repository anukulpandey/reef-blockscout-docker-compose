defmodule Evision.RGBD do
  ...
  def depthTo3d([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:points3d,:mask,:K,:depth])
  ...
  def depthTo3dSparse([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:points3d,:in_points,:depth,:in_K])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def registerDepth([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:registeredDistCoeffs,:outputImagePlaneSize,:depthDilation,:Rt,:registeredDepth,:registeredCameraMatrix,:unregisteredCameraMatrix,:unregisteredDepth])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def rescaleDepth([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:out,:depth_factor,:in,:depth])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
  ...
  def warpFrame([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:image,:cameraMatrix,:depth,:mask,:Rt,:warpedMask,:distCoeff,:warpedImage,:warpedDepth])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
