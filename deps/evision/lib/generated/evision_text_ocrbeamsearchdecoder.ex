defmodule Evision.Text.OCRBeamSearchDecoder do
  ...
  def create([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:vocabulary,:classifier,:mode,:beam_size,:transition_probabilities_table,:emission_probabilities_table])
  ...
  def run([{arg, _} | _] = named_args) when is_atom(arg) do
    named_args = Keyword.validate!(named_args, [:component_level,:image,:min_confidence,:mask])
    Enum.map(named_args, fn {named_arg, value} -> {named_arg, Evision.Internal.Structurise.from_struct(value)} end)
